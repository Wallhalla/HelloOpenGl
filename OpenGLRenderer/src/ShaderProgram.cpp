...
{
	GLCall(glUniform1i(GetUniformLocation(uniformName), i));
}

void ShaderProgram::SetUniform4f(const std::string& uniformName, float x, float y, float z, float a)
{		
	GLCall(glUniform4f(GetUniformLocation(uniformName), x, y, z, a));
}
...
int ShaderProgram::GetUniformLocation(const std::string& uniformName)
{
	...
	GLCall(int location = glGetUniformLocation(_id, uniformName.c_str()));
